...
pacstrap_pkgs="${pacstrap_pkgs} zsh zsh-completions tmux ghostty ghostty-terminfo ghostty-shell-integration" # my preferred shell & terminal
...
pacstrap_pkgs="${pacstrap_pkgs} tree-sitter tree-sitter-cli" # nvim treesitter
...
if lscpu | grep Model | grep Intel; then
  ...
  pacstrap_pkgs="${pacstrap_pkgs} amd-ucode vulkan-radeon mesa libva-utils"
fi
